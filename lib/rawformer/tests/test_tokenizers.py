"""Tests for BPE and WordPiece tokenizers."""

import tempfile
from pathlib import Path

from rawformer.tokenizers.bpe import BPETokenizer
from rawformer.tokenizers.wordpiece import WordPieceTokenizer
...
class TestWordPieceTrain:
    ...
    def test_continuation_tokens_present(self) -> None:
        """Characters that appear mid-word should have ## variants."""
        tok = WordPieceTokenizer()
        tok.train(_CORPUS, vocab_size=50)
        # 'h' appears as second char in "the" -> "##h" should exist
        assert "##h" in tok.vocab
...
class TestWordPieceEncode:
    ...
    def test_encode_starts_with_cls(self) -> None:
        tok = WordPieceTokenizer()
        tok.train(_CORPUS, vocab_size=50)
        ids = tok.encode("the cat")
        assert ids[0] == tok.cls_token_id

    def test_encode_ends_with_sep(self) -> None:
        tok = WordPieceTokenizer()
        tok.train(_CORPUS, vocab_size=50)
        ids = tok.encode("the cat")
...
class TestWordPiecePersistence:
    def test_save_and_load_roundtrip(self) -> None:
        tok = WordPieceTokenizer()
        tok.train(_CORPUS, vocab_size=50)
